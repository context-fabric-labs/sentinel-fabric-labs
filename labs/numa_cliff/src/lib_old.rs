...
pub async fn run_lab(args: Args) -> Result<NumaCliffResult, Box<dyn std::error::Error>> {
    ...
    for _ in 0..num_threads {
        ...
        let handle = tokio::spawn(async move {
            ...
            for i in 0..(args.requests / num_threads + 1) {
                ...
                // Access memory in a pattern that might trigger NUMA effects
                let base_index = (i * 1000) % memory_size;
                for j in 0..100 {
                    let index = (base_index + j) % memory_size;
    ...
    // In a real implementation, we would capture actual NUMA/perf counters
    let remote_accesses = 0;
...
mod tests {
    ...
    async fn test_numa_access_patterns() {
        // Test that NUMA access patterns work
        let args = Args {
